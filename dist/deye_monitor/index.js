...
async function internalGet(env, path) {
...
async function listStations(env) {
  if (!passwordReady(env) && env.DEYE_SESSION_TOKEN) {
    const d2 = await internalPost(env, "/maintain-s/operating/station/v2/search?page=1&size=50&order.direction=ASC&order.property=name", {});
    return (d2.data || []).map((x) => {
      const s = x.station || x;
  ...
  return (d.stationList || d.list || []).map((s) => ({ id: s.id || s.stationId, name: s.name, capacity: s.installedCapacity }));
}
...
async function getLatestInternal(env) {
  const id = await getStationId(env);
  const d = await internalGet(env, `/maintain-s/fast/system/${id}`);
  ...
  return {
    ...
    updatedAt: Number(d.lastUpdateTime || Math.floor(Date.now() / 1e3))
  };
}
...
async function getLatest(env) {
  if (passwordReady(env)) {
    try {
      return await getLatestOpen(env);
    } catch (e) {
      if (!env.DEYE_SESSION_TOKEN) throw e;
    }
  }
...
async function listDevices(env, stationId) {
  const id = await getStationId(env);
  if (passwordReady(env) || !env.DEYE_SESSION_TOKEN) {
    const d2 = await apiPost(env, "/station/device", { page: 1, size: 50, stationIds: [Number(id)] });
    return d2.deviceListItems || d2.list || d2.data || [];
  }
...
async function histFromD1(env, range) {
  if (range === "day") {
    ...
    const { results: results2 } = await env.DB.prepare("SELECT ts, gen_power, use_power, soc FROM samples WHERE ts>=? ORDER BY ts").bind(start).all();
