...
export interface Device {
  ...
  battChannelsActive?: number[];                         // DC channels EVER observed carrying current (undefined = no history yet)
}
...
export interface BatteryHealth { coverage: { samples: number; withSoc: number; unknownSoc: number }; sn?: string | null; lastEstimateDay?: string | null; days: number; ratedKwh: number | null; capKwh: number | null; soh: number | null; cycles: number | null; dischargeKwh: number; dod: { avg: number | null; minSoc: number | null; lowDays: number }; trend: { day: string; capKwh: number; segments: number }[]; firstCapKwh: number | null; nominalV?: number | null; ratedAh?: number | null; }
